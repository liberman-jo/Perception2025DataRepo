...
# Use the extracted folder structure
...
read_trc <- function(path) {
  ...
  # Fix TRC column naming: markers are followed by 3 blank columns for X, Y, Z
  nms <- names(df)
  for (i in seq_along(nms)) {
    if (i > 2 && (is.na(nms[i]) || trimws(nms[i]) == "")) {
      # Find previous marker name
...
extract_trialnum <- function(filename) {
  # Multiple patterns for different participants
...
# CRITICAL FIX: The Excel file is read with rows shifted
# Row 1 in R = condition descriptions (Excel row 4)
# Row 2 in R = condition keys (Excel row 5)
# Rows 3-7 in R = participants (Excel rows 6-10)
...
parse_trials_cell <- function(x) {
  ...
  # Fix typos like "28, 29. 30"
  x <- gsub("\\.", ",", as.character(x))
  as.integer(str_extract_all(x, "\\d+")[[1]])
}

# Read participant rows (rows 3-7 in the data frame)
...
cat(sprintf("Loaded condition map: %d trial assignments for %d participants\n", 
...
convert_trcs <- function(trc_dir, participant) {
  ...
  for (f in trcs) {
    ...
    write_csv(df, out_path, progress=FALSE)
...
for (i in 1:nrow(all_files_mapped)) {
  ...
  if (i %% 20 == 0) cat(sprintf("  Processing file %d/%d...\n", i, nrow(all_files_mapped)))
...
# For each trial, compute mean and min distances
...
cat(sprintf("Trial metrics computed: %d trials\n", nrow(trial_metrics)))
...
make_package <- function(metric=c("meanAB","minAB")) {
  ...
  if (metric == "meanAB") {
    val <- "meanAB"
    val_AminusB <- "meanAminusB"
  } else {
    val <- "minAB"
    val_AminusB <- "minAminusB"
  }
  
  # Block summaries (mean over 3 trials per condition per participant)
  ...
  cat(sprintf("Block summary: %d blocks across %d participants\n", 
  ...
  out_root <- file.path(output_dir, paste0("final_plots_all_participants_", toupper(metric_name)))
  dir_create(out_root, recurse=TRUE)
  
  cat("Generating plots...\n")
  
  # 01: Composition effect
  p1 <- block_summary %>%
    group_by(Composition) %>%
    summarise(mean_val=mean(metric, na.rm=TRUE),
              se=sd(metric, na.rm=TRUE)/sqrt(sum(is.finite(metric))),
              .groups="drop") %>%
    ...
    labs(
      title=paste0("Composition effect (pooled) on block meanAB — metric=", metric_name),
  ...
  p3 <- collider_eff %>%
    ...
    labs(title=paste0("Collider effect on within-block Δ (Trial3−Trial1) — metric=", metric_name),
  ...
  p4 <- block_summary %>%
    ...
    geom_errorbar(aes(ymin=mean_val-se, ymax=mean_val+se), width=0.2) +
    labs(title=paste0("Mode effect (pooled) on block meanAB — metric=", metric_name),
  ...
  # 05: Mode effect on within-block delta
  p5 <- wb %>%
    group_by(Mode) %>%
    ...
    geom_errorbar(aes(ymin=mean_val-se, ymax=mean_val+se), width=0.2) +
    geom_hline(yintercept=0, linetype="dashed") +
    labs(title=paste0("Mode effect (pooled) on within-block Δ — metric=", metric_name),
         x="Mode", y="Δ meanAB") + theme_pro()
  save_png(p5, file.path(out_root, "05_mode_effect_withinblock_delta_meanAB_all.png"), 10, 6)
  
  # 06: Reproducibility scatter - within-block delta
  ...
  p6 <- ggplot(wb_long, aes(x=consensus, y=delta_meanAB)) +
    ...
    labs(title=paste0("Repro: within-block Δ vs consensus (all points) — metric=", metric_name),
  ...
  # 07: Reproducibility scatter - block means
  ...
  p7 <- ggplot(bm_long, aes(x=consensus, y=metric)) +
    ...
    geom_smooth(method="lm", se=TRUE, color="red") +
    labs(title=paste0("Repro: block meanAB vs consensus — metric=", metric_name),
  ...
  p8 <- ggplot(early_late, aes(x=EarlyLate, y=mean_val)) +
    ...
    geom_errorbar(aes(ymin=mean_val-se, ymax=mean_val+se), width=0.2) +
    labs(title=paste0("Early vs Late blocks (pooled) — metric=", metric_name),
  ...
  p9 <- block_summary %>%
    ...
    labs(title=paste0("BoxA−BoxB by collider (block means) — metric=", metric_name),
         x="Collider", y="Block mean(A−B)") + theme_pro()
  save_png(p9, file.path(out_root, "09_boxAminusB_by_collider_blockmeans_all.png"), 12, 6)
  
  # 10: Within-block delta A-B
  p10 <- wb_ab %>%
    ggplot(aes(x=factor(Collider), y=delta_AminusB)) +
    geom_hline(yintercept=0, linetype="dashed") +
    geom_boxplot(alpha=0.6, fill="orange") +
    labs(title=paste0("Within-block Δ(A−B) by collider — metric=", metric_name),
  ...
  p11 <- ggplot(traj, aes(x=ConditionID, y=mean_val, color=factor(TrialInBlock))) +
    ...
    geom_point(size=2) +
    labs(title=paste0("Block trajectory by trial-in-block (pooled) — metric=", metric_name),
         x="ConditionID (block order)", y="Trial metric value", color="Trial in Block") +
    theme_pro()
  ...
  # Sanity check JSON
  participant_trial_counts <- trial_metrics %>%
    group_by(Participant) %>%
    summarise(n_trials = n(), .groups="drop") %>%
    deframe()
  
  missing_trials <- trial_cond_map %>%
  ...
  sanity <- list(
    ...
    n_trials_per_participant = as.list(participant_trial_counts),
    missing_trials_per_participant = as.list(missing_trials)
  )
...
cat("\n========================================\n")
cat("ANALYSIS COMPLETE! (NOW WITH P5!)\n")
cat("========================================\n")
...
cat("  - ", out_mean_dir, "\n")
cat("  - ", out_min_dir, "\n")
cat("\nAll 5 participants included: P1.2, P2, P3, P4, P5\n")
